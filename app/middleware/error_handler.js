...
module.exports = (option, app) => {
  return async function(ctx, next) {
    try {
      ...
      const msg = status === 500 && app.config.env === "prod" ? "服务器繁忙" : err.errors[0].message
      // 从 error 对象上读出各个属性，设置到响应中
      ctx.body = {
        code: status, // 服务端自身的处理逻辑错误(包含框架错误500 及 自定义业务逻辑错误533开始 ) 客户端请求参数导致的错误(4xx开始)，设置不同的状态码
        msg: ctx.gettext(msg)
      }
      // if (err instanceof app.Sequelize.UniqueConstraintError) {
      //   // console.log("sequlize的错误", err.errors[0])
